...
RUN apt-get update && \
    apt-get install -y python3 make git gdb gdb-multiarch gcc-multilib-arm-linux-gnueabi gcc-x86-64-linux-gnu g++-x86-64-linux-gnu qemu;

RUN for file in /usr/bin/x86_64-linux-gnu-*; do link_name=$(basename $file | sed 's|x86_64-linux-gnu-||'); ln -s $file /usr/bin/$link_name; done
